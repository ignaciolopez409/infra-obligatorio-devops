...
resource "aws_iam_role" "eksClusterRole" {
  name = "eksClusterRole"
  assume_role_policy = jsonencode(data.aws_iam_policy.AmazonEKSClusterPolicy.policy)
}

resource "aws_iam_role" "eksWorkerRole" {
  name = "eksWorkerRole"
  assume_role_policy = jsonencode(data.aws_iam_policy.AmazonNodePolicy.policy)
}
